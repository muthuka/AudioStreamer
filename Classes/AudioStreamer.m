...
NSString * const ASStatusErrorNotification = @"ASStatusErrorNotification";
...
- (void)failWithErrorCode:(AudioStreamerErrorCode)anErrorCode
{
	@synchronized(self)
	{
        ...
        NSNotification *notification = [NSNotification notificationWithName:ASStatusErrorNotification object:self];
	...
	@synchronized(self)
	{
		...
		if (fileLength > 0 && seekByteOffset > 0)
		{
			CFHTTPMessageSetHeaderFieldValue(message, CFSTR("Range"),
				(__bridge CFStringRef)[NSString stringWithFormat:@"bytes=%ld-%ld", (long)seekByteOffset, fileLength - 1]);
			discontinuous = vbr;
		}
